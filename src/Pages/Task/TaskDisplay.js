    ...
    return (
        <div className='w-full h-full flex flex-col gap-3'>
            {tasks && showAllTask === true ? tasks?.all?.length !== 0 ? tasks?.all?.map(({ id, title, task_date }, index) => (
                <div key={index} className="w-full h-auto p-[10px] bg-main-blue rounded-md flex justify-between items-center cursor-pointer" onClick={() => navigate(`${id}`)} >
                    <p className="w-auto font-medium text-[15px]">{title}</p>
                    <p className="w-auto font-medium text-[15px]">Task Date: {task_date}</p>
                </div>
            )) : <p>No Tasks</p> : tasks?.todays?.length !== 0 ? tasks?.todays?.map(({ id, title, task_date }, index) => (
                <div key={index} className="w-full h-auto p-[10px] bg-main-blue rounded-md flex cursor-pointer justify-between items-center" onClick={() => navigate(`${id}`)} >
                    <p className="w-auto font-medium text-[15px]">{title}</p>
                    <p className="w-auto font-medium text-[15px]">Task Date: {task_date}</p>
                </div>
            )) : <p> No tasks</p>
